...
namespace dash
{
    ...
    int BGJobAdapter::runInBackground(struct job *jp, const std::string &cmd, char *const argv[])
    {
        ...
        // 创建输出文件
        std::string output_file = "bg_output_" + std::to_string(getpid()) + "_" + std::to_string(time(NULL)) + ".txt";
        ...
        if (pid < 0) {
            ...
            if (write(pipefd[1], &pid2, sizeof(pid2)) != sizeof(pid2)) {
                // 写入失败，但继续执行
            }
            close(pipefd[1]);
        ...
        // 从管道读取第二子进程的PID
        pid_t child_pid = -1;
        ssize_t bytes_read = read(pipefd[0], &child_pid, sizeof(child_pid));
        close(pipefd[0]);
        ...
        if (bytes_read == sizeof(child_pid)) {
            ...
            if (job_control_) {
                ...
                int job_id = job_control_->addJob(cmd, child_pid);
        ...
        // 打印后台作业信息
        if (child_pid > 0) {
            std::cout << "[1] " << child_pid << " (守护进程模式，输出重定向到" << output_file << ")" << std::endl;
        } else {
            std::cout << "[1] ？ (守护进程模式，输出重定向到" << output_file << ")" << std::endl;
        }
